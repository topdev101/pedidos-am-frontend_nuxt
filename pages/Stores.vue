<template>
    <div class="main-container container-fluid">
        <div class="page-header">
            <h4 class="page-title mb-0"><i class="mdi mdi-store-outline menu-icon"></i> {{$t('page.store')}}</h4>
            <nav aria-label="breadcrumb mb-0">
                <ol class="breadcrumb mb-0">
                    <li class="breadcrumb-item"><router-link :to="{name: 'dashboard'}">{{$t('page.home')}}</router-link></li>
                    <li class="breadcrumb-item active" aria-current="page">{{$t('page.store')}}</li>
                </ol>
            </nav>
        </div>
        <div class="row">
            <div class="col-12">
                <div class="card card-body page-content-card">
                    <div class="d-md-flex">
                        ...
                        <button type="button" class="btn btn-success ms-auto mb-2" @click="addStore()"><i class="ti-plus"></i>{{$t('page.add_new')}}</button>
                    </div>
            ...
            <ui-modal name="store_modal"
                :title="form_mode == 'edit' ? $t('page.edit_store') : $t('page.add_store')"
            >
                <store-form :mode="form_mode"
                     :id="selected_store ? selected_store.id : ''"
                     @submit="storeSubmitted"
                />
            </ui-modal>
        </div>
    </div>
        ...
        deleteUser(item, index) {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    this.axios.delete(`/store/delete/${item.id}`).then(response => {
                        if (response.data.status == 'Success') {
