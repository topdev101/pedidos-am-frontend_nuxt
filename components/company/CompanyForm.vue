<template>
    <form action="" method="post" @submit.prevent="submit()">
        ...
        <hr class="my-2" />
        <div class="text-end">
            <button type="button" class="btn btn-danger" @click="closeModal('company_modal')"><i class="fe fe-x me-1"></i>{{$t('page.close')}}</button>
...
export default {
    ...
    methods: {
        getDetail() {
            this.axios.get(`/company/get_detail/${this.id}`).then(response => {
        ...
        submit() {
            this.form.post('company/save').then(response => {
                if (response.data.status == 'Success') {
